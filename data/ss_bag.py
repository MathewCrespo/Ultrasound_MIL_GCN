...
class SSBags(Dataset):
    ...
    def s_search(self,img, scale=1, sigma=0.9,min_size=100):
        ...
        #print(len(bbox))
        return torch.stack([ x for x in bbox], dim = 0), len(bbox)
        ...
        if self.mix_mode == 0:  # no operations
            for path in grey_img_path:
                ...
                if self.selective_mode:
                    grey_img, region_num = self.s_search(grey_img)
...
if __name__=="__main__":
    ...
    root =  '/remote-home/my/Ultrasound_CV/data/MergePhase1/test_0.3/train'
