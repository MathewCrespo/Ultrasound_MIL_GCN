...
class MILCTrainer(object):
    ...
    def train(self):
        ...
        pred = []
        target = []
        for batch_data, clinical_info, batch_label in (tqdm(self.train_loader, ascii=True, ncols=60)):
            # reset gradients
            ...
            for idx, data in enumerate(batch_data):
                ...
                loss, prob_label, _ = self.net.calculate_objective(data, clinical_info, bag_label)
    ...
    def test(self):
        ...
        pred = []
        prob = []
        for batch_data, clinical_info, batch_label in tqdm(self.test_loader, ascii=True, ncols=60):
            for idx, data in enumerate(batch_data):
                data = data.cuda()
                clinical_info = clinical_info.cuda()
                bag_label = batch_label[idx].cuda()

                loss, prob_label, attention_weights = self.net.calculate_objective(data, clinical_info, bag_label)
